...
export default NextAuth({
  ...
  providers: [
    ...
    CredentialsProvider({
        ...
        let initials = userName.length > 1 ? userName[0][0].toUpperCase()+' '+userName[1][0].toUpperCase() : userName[0][0].toUpperCase();

        return Promise.resolve({   id: user.id, email: user.email, role: user.role, initials: initials });
...
if (!user) {
  ...
  const obj={
    ...
    return Promise.resolve({ id: insert.insertId, email: data.email, role: 'user'  });
